...
export default function SellerAddressesPage() {
  ...
  const fetchAddresses = async (page: number) => {
    try {
      ...
      const response = await getSellerAddresses(page);
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this address?")) {
      try {
        await deleteSellerAddress(Number(id));
        fetchAddresses(currentPage);
      } catch (error) {
  ...
  const handleSetDefault = async (id: string) => {
    try {
      await setSellerAddressDefault(Number(id));
      fetchAddresses(currentPage);
    } catch (error) {
  ...
  const handlePageChange = (page: number) => {
    router.push(`/seller/addresses?page=${page}`);
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage your business locations and shipping origins
            </p>
          </div>
          <Button
            onClick={() => router.push("/seller/addresses/create")}
        ...
        {loading && addresses.length === 0 ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 mt-1 mb-6">
              Add a business address to start shipping products
            </p>
            <Button
              onClick={() => router.push("/seller/addresses/create")}
              variant="outline"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {addresses.map((address) => (
                <AddressCard
                  key={address.id}
                  address={address}
                  onEdit={() => router.push(`/seller/addresses/${address.id}`)}
